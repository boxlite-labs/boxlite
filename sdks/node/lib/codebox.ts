...
export class CodeBox extends SimpleBox {
   ...
   * @param timeout - Execution timeout in seconds (not yet implemented)
   *
   * @returns Promise resolving to execution output (stdout + stderr)
   *
   * @example
   * ```typescript
   * const codebox = new CodeBox();
   ...
   * ```
   */
  async run(code: string, _timeout?: number): Promise<string> {
